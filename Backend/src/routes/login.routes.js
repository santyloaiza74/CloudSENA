...
// Registro de usuarios accesible por todos
router.post('/register', async (req, res) => {
    const { email, password, nombre, documento, ficha, gestor } = req.body;
    const emaildup = await loginSchema.findOne({ email: email });
    if (emaildup) return res.status(400).json({ message: "El email ya se encuentra registrado" });
    const documentodup = await loginSchema.findOne({ documento: documento });
    if (documentodup) return res.status(400).json({ message: "El documento ya se encuentra registrado" });
...
// Modificación de usuarios accesible por gestor, admin y superadmin
router.put('/:id', validateToken, verifyRole('gestor'), verifyRole('admin'), verifyRole('superadmin'), async (req, res) => {
    const { id } = req.params;
...
// Eliminación de usuarios accesible por gestor, admin y superadmin
router.delete('/:id', validateToken, verifyRole('gestor'), verifyRole('admin'), verifyRole('superadmin'), async (req, res) => {
    const { id } = req.params;
    try {
...
// Autenticación de usuarios accesible por todos
